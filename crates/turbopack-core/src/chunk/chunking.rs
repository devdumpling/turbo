...
async fn chunking(
              ...
              for &reference in chunk_item.references().await?.iter() {
                  if let Some(chunkable) = ChunkableAssetReferenceVc::resolve_from(reference).await? {
                      match &*chunkable.chunking_type().await? {
                          ...
                          Some(ChunkingType::Separate) => {
                              for &asset in &*chunkable.resolve_reference().primary_assets().await? {
                                  results.push(ResultItem::Separate(asset));
                              }
                          }
                          Some(ChunkingType::SeparateAsync) => {
                              for &asset in &*chunkable.resolve_reference().primary_assets().await? {
                                  results.push(ResultItem::SeparateAsync(asset));
                              }
                          }
                      }
                  } else {
    ...
    let mut separate_assets = Vec::new();
    ...
    for item in results.into_inner().into_iter() {
        match item {
            ...
            ResultItem::Separate(asset) => {
                separate_assets.push(asset);
            }
            ResultItem::SeparateAsync(asset) => {
